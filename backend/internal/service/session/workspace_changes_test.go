...
func TestWorkspaceFileDiff_DeletedFile(t *testing.T) {
	...
	res, err := svc.WorkspaceFileDiff(context.Background(), "s1", "gone.go", DiffBaseTarget)
	if err != nil {
		t.Fatal(err)
	}
	if !res.Available {
...
func TestWorkspaceFileDiff_MarksSkippedRegions(t *testing.T) {
	...
	res, err := svc.WorkspaceFileDiff(context.Background(), "s1", "wide.go", DiffBaseTarget)
	if err != nil {
		t.Fatal(err)
	}
...
func TestWorkspaceFileDiff_UntrackedFile(t *testing.T) {
	...
	res, err := svc.WorkspaceFileDiff(context.Background(), "s1", "untracked.go", DiffBaseTarget)
	if err != nil {
		t.Fatal(err)
	}
	if !res.Available {
...
func TestWorkspaceFileDiff_ConfinesPathBeforeGit(t *testing.T) {
	...
	if _, err := svc.WorkspaceFileDiff(context.Background(), "s1", "../../etc/passwd", DiffBaseTarget); err != nil {
		t.Fatal(err)
	}
...
func TestWorkspaceFileDiff_AbsolutePathYieldsNothing(t *testing.T) {
	...
	for _, p := range []string{"/etc/passwd", "~/.ssh/id_rsa", "~", ""} {
		res, err := svc.WorkspaceFileDiff(context.Background(), "s1", p, DiffBaseTarget)
		if err != nil {
			t.Fatalf("%q: %v", p, err)
		}
		if res.Available {
