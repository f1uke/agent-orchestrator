...
func seedRunCheck(t *testing.T, svc *Service, store *fakeStore, id string, seq int, verdict domain.SmokeVerdict, note, mime, data string) {
	t.Helper()
	store.checks[id] = domain.SmokeCheck{ID: id, SessionID: "w1", Seq: seq, Name: "Check " + id, Verdict: domain.SmokePending, Evidence: []domain.SmokeEvidence{}}
...
func TestPostToJira_OnlyRunRowsAndUploadsEvidence(t *testing.T) {
	...
	// header row + 3 run rows = 4 tableRow nodes; the pending row is absent.
	...
	// Evidence cell links to the uploaded attachment content URLs.
	if !strings.Contains(doc, "/secure/attachment/1/c1-shot") {
		t.Errorf("doc missing link to c1 evidence: %s", doc)
	}
	// The image is embedded inline; the video is link-only (no second mediaSingle).
	...
	// Attachments carried the right key + bytes.
