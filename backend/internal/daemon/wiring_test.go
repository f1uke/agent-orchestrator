...
func TestWiring_StartSessionBuildsSessionService(t *testing.T) {
	...
	svc, reviewSvc, lc, err := startSession(cfg, rt, store, lcm, messenger, telemetryadapter.NoopSink{}, spawnConfirm, promptOverrides, log)
	if err != nil {
...
func TestStartTrackerIntake_RunsEvenWithoutEnabledProjects(t *testing.T) {
	...
	svc, _, _, err := startSession(cfg, rt, store, lcm, messenger, telemetryadapter.NoopSink{}, spawnConfirm, promptOverrides, log)
	if err != nil {
		t.Fatalf("startSession: %v", err)
	}
