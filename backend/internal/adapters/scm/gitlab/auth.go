...
// TokenSource yields a GitLab bearer token on demand. Production wires this
...
// GlabTokenSource shells out to `glab auth status --show-token` when env
// vars are not configured. It memoizes the result for TokenTTL so we don't
// fork-exec on every request, but the Client invalidates the cache on auth
// failures so a rotated token is picked up on the next call. Tests inject
// Glab so the glab binary is never required.
type GlabTokenSource struct {
	// Host is the GitLab hostname passed to `glab auth status --hostname`.
	Host string
	// Glab is the shell-out hook. Production leaves this nil and falls back
	// to `glab auth status --show-token --hostname <host>`; tests inject a
...
// Token returns the cached token if still fresh, otherwise re-runs glab.
...
// parseGlabToken extracts the token from `glab auth status --show-token`
// output. It scans for a line containing "Token" and returns the trimmed
// text after the last colon, skipping masked tokens (which contain "*").
