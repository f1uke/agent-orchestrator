...
-- name: SetSessionAutoResolve :execrows
-- Set (or clear) the per-session auto-resolve-on-reply gate. NULL = OFF (there is
-- no global default to inherit). Bumps updated_at so the sessions_cdc_update
