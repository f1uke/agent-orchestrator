...
// SetSessionAutoResolve sets (or clears) the per-session auto-resolve-on-reply
// gate. A nil override means OFF and is stored as SQL NULL (there is no global
// default to inherit). Returns ok=false when the session id does not exist.
func (s *Store) SetSessionAutoResolve(ctx context.Context, id domain.SessionID, override *bool, updatedAt time.Time) (bool, error) {
	s.writeMu.Lock()
	defer s.writeMu.Unlock()
	rows, err := s.qw.SetSessionAutoResolve(ctx, gen.SetSessionAutoResolveParams{
		ID:                 id,
		AutoResolveOnReply: boolPtrToNullInt64(override),
		UpdatedAt:          updatedAt,
	})
	if err != nil {
		return false, fmt.Errorf("set auto-resolve for session %s: %w", id, err)
	}
	return rows > 0, nil
}
...
func (s *Store) SetSessionIssueBinding(ctx context.Context, id domain.SessionID, issueID, displayName string, updatedAt time.Time) (bool, error) {
	s.writeMu.Lock()
	defer s.writeMu.Unlock()
	rows, err := s.qw.SetSessionIssueBinding(ctx, gen.SetSessionIssueBindingParams{
		ID:          id,
		IssueID:     domain.IssueID(issueID),
		DisplayName: displayName,
		UpdatedAt:   updatedAt,
	})
	if err != nil {
		return false, fmt.Errorf("set issue binding for session %s: %w", id, err)
	}
	return rows > 0, nil
}
...
func rowToRecord(row gen.Session) domain.SessionRecord {
	return domain.SessionRecord{
		...
		AutoResolveOnReply: nullInt64ToBoolPtr(row.AutoResolveOnReply),
		IsTodo:             row.IsTodo,
		IsSuspended:        row.IsSuspended,
		SleepReason:        domain.SleepReason(row.SleepReason),
		WokenBy:            domain.WokenBy(row.WokenBy),
		...
		// terminated_at NULL (or an empty source on a row written before AO kept
		// this account) maps to the zero Termination, which reads as "no account
		// of how this ended" rather than being guessed at.
		Termination: domain.Termination{
			Source:         row.TerminationSource,
			Reason:         row.TerminationReason,
			LastState:      row.TerminationLastState,
			TranscriptPath: row.TerminationTranscriptPath,
			At:             nullTimeToTime(row.TerminatedAt),
		},
...
func recordToInsert(rec domain.SessionRecord, num int64) gen.InsertSessionParams {
	...
	return gen.InsertSessionParams{
		...
		AutoResolveOnReply:        boolPtrToNullInt64(rec.AutoResolveOnReply),
		IsTodo:                    rec.IsTodo,
		...
		SleepReason:               string(rec.SleepReason),
		WokenBy:                   string(rec.WokenBy),
		...
		TerminationTranscriptPath: rec.Termination.TranscriptPath,
		TerminatedAt:              timeToNullTime(rec.Termination.At),
