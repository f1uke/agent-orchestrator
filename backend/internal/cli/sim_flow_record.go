...
// team's suite.
//
// It lives under `ao sim flow`, beside `check` and `run`, because a flow is
// what it produces: it used to be `ao sim record`, which every reader took for
// screen recording, and that name now belongs to the command that really does
// record the screen (sim_record.go). There is deliberately NO alias for the old
// spelling - a spelling that still works is the confusion the move exists to
// remove.
//
// The daemon side keeps its own names (`sim-recordings`, domain.SimRecording):
// they are internal, and the shipped HTTP route and the table behind it are a
// contract this rename has no reason to break.
//
...
func newSimFlowRecordCommand(ctx *commandContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "record",
	...
	cmd.AddCommand(newSimFlowRecordStartCommand(ctx))
	cmd.AddCommand(newSimFlowRecordStatusCommand(ctx))
	cmd.AddCommand(newSimFlowRecordStopCommand(ctx))
	return cmd
}

// --- ao sim flow record start ----------------------------------------------------

// simFlowRecordStartResult is the `ao sim flow record start --json` payload.
type simFlowRecordStartResult struct {
	UDID          string    `json:"udid"`
...
func newSimFlowRecordStartCommand(ctx *commandContext) *cobra.Command {
	var opts struct {
		udid string
		name string
		json bool
  ...
  ao sim flow record start --name "sign up flow"`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return writeSimFlowRecordStart(cmd.OutOrStdout(), result)
		},
	}
	f := cmd.Flags()
...
func (c *commandContext) startSimRecording(ctx context.Context, udid, name string) (simFlowRecordStartResult, error) {
	sessionID, err := simSessionID("`ao sim flow record start`")
	if err != nil {
		return simFlowRecordStartResult{}, err
	}
	...
	if err := c.postJSON(ctx, path, body, &res); err != nil {
		return simFlowRecordStartResult{}, c.explainSimRecordingRefusal(device, err)
	}
	return simFlowRecordStartResult{
...
func writeSimFlowRecordStart(out io.Writer, r simFlowRecordStartResult) error {
	...
	_, err := fmt.Fprintln(out,
		"Drive it with `ao sim tap`/`swipe`/`drag`/`type`/`button`, or the Device tab. Stop it with `ao sim flow record stop`.")
	return err
}

// --- ao sim flow record status ---------------------------------------------------

// simFlowRecordStatusResult is the `ao sim flow record status --json` payload.
type simFlowRecordStatusResult struct {
	UDID       string `json:"udid"`
	DeviceName string `json:"deviceName"`
	Runtime    string `json:"runtime"`
...
// Structurally alike but unrelated to `ao sim record status` - see the note on
// newSimRecordStatusCommand in sim_record.go for why they are not one factory.
//
//nolint:dupl // structurally alike, unrelated commands.
func newSimFlowRecordStatusCommand(ctx *commandContext) *cobra.Command {
	var opts struct {
		udid string
		json bool
	}
	cmd := &cobra.Command{
		...
		Long: "Report a device's recording: whether one is open, when it started, and how many " +
			...
			"and exits 0.",
		Example: `  ao sim flow record status`,
		Args:    noArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return writeSimFlowRecordStatus(cmd.OutOrStdout(), result)
		},
	}
	f := cmd.Flags()
...
func (c *commandContext) simRecordingStatus(ctx context.Context, udid string) (simFlowRecordStatusResult, error) {
	sessionID, err := simSessionID("`ao sim flow record status`")
	if err != nil {
		return simFlowRecordStatusResult{}, err
	}
	device, err := c.resolveBootedSimDevice(ctx, udid)
	if err != nil {
		return simFlowRecordStatusResult{}, err
	...
	if err := c.getJSON(ctx, path, &res); err != nil {
		...
		if errors.As(err, &apiErr) && apiErr.ErrorBody.Code == "SIM_NOT_FOUND" {
			return simFlowRecordStatusResult{UDID: device.UDID, DeviceName: device.Name, Runtime: device.Runtime}, nil
		}
		return simFlowRecordStatusResult{}, err
	}

	result := simFlowRecordStatusResult{
...
func writeSimFlowRecordStatus(out io.Writer, r simFlowRecordStatusResult) error {
	if !r.Found {
...
// --- ao sim flow record stop -----------------------------------------------------

// simFlowRecordStopResult is the `ao sim flow record stop --json` payload.
type simFlowRecordStopResult struct {
	UDID       string `json:"udid"`
	DeviceName string `json:"deviceName"`
	Runtime    string `json:"runtime"`
	Path       string `json:"path"`
...
func newSimFlowRecordStopCommand(ctx *commandContext) *cobra.Command {
  ...
  ao sim flow record stop --out /tmp/flow.yaml --json`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return writeSimFlowRecordStop(cmd.OutOrStdout(), result)
		},
	}
	f := cmd.Flags()
...
func (c *commandContext) stopSimRecording(ctx context.Context, udid, out, entry string) (simFlowRecordStopResult, error) {
	...
	if trimmed := strings.TrimSpace(out); trimmed != "" {
		...
		if err != nil {
			return simFlowRecordStopResult{}, fmt.Errorf("resolve --out path: %w", err)
		}
		query.Set("out", abs)
	}
	...
	if err := c.deleteJSON(ctx, path, &res); err != nil {
		return simFlowRecordStopResult{}, c.explainSimRecordingStopFailure(device, err)
	}
	if res.Flow == nil {
		return simFlowRecordStopResult{}, fmt.Errorf(
			"recording stopped on %s with %d step(s) captured, but the daemon wrote no flow file",
			device.Label(), res.StepCount)
	}
	return simFlowRecordStopResult{
		UDID: device.UDID, DeviceName: device.Name, Runtime: device.Runtime,
		Path: res.Flow.Path, StepCount: res.Flow.Steps, ReviewCount: res.Flow.Review, Bytes: int(res.Flow.Bytes),
	}, nil
}

func writeSimFlowRecordStop(out io.Writer, r simFlowRecordStopResult) error {
...
// into the sentence that says what to do about it. `ao sim flow record start`
...
func (c *commandContext) explainSimRecordingRefusal(device simDevice, err error) error {
	...
	case "already_open":
		return fmt.Errorf("%s already has a recording open, so nothing was started.\n"+
			"Run `ao sim flow record status` to see what it has captured, or `ao sim flow record stop` to close it first",
			device.Label())
	case "leased_by_other":
		...
		return fmt.Errorf("%s is leased by @%s%s, so this session may not record it.\n"+
			"`ao sim flow record start` never claims a device - wait for the lease to lapse, or ask @%s to run `ao sim release`",
			device.Label(), holder, left, holder)
	default:
		return fmt.Errorf("%s is not claimed by this session, so it may not be recorded.\n"+
			"Run `ao sim claim` first - `ao sim flow record start` never claims a device on your behalf",
			device.Label())
	}
}
...
func (c *commandContext) explainSimRecordingStopFailure(device simDevice, err error) error {
	...
	return fmt.Errorf("nothing is being recorded on %s, so there is nothing to stop.\n"+
		"Run `ao sim flow record start` first", device.Label())
}
