...
func TestSimFlowRecordStart_RequiresALeaseAndNeverAcquiresOne(t *testing.T) {
	...
	out, _, err := executeCLI(t, simLeaseDeps(t, bootedProMaxOnly(t), fakePNG), "sim", "flow", "record", "start")
	if err == nil {
	...
	for _, call := range daemon.calls {
		if strings.Contains(call, "/sim-leases") && !strings.Contains(call, "/sim-recordings/") {
			t.Fatalf("`ao sim flow record start` must never acquire a lease itself: %v", daemon.calls)
		}
	}
}

func TestSimFlowRecordStart_RefusedWhenLeasedByOtherNamesTheHolder(t *testing.T) {
	t.Setenv("AO_SESSION_ID", "mer-9")
	cfg := setConfigEnv(t)
	daemon := newSimDaemon(t, cfg)
	daemon.recordStartStatus = http.StatusConflict
	...
	_, _, err := executeCLI(t, simLeaseDeps(t, bootedProMaxOnly(t), fakePNG), "sim", "flow", "record", "start")
	if err == nil {
...
func TestSimFlowRecordStart_RefusedWhenAlreadyOpen(t *testing.T) {
	t.Setenv("AO_SESSION_ID", "mer-9")
	cfg := setConfigEnv(t)
	daemon := newSimDaemon(t, cfg)
	daemon.recordStartStatus = http.StatusConflict
	...
	msg := err.Error()
	for _, want := range []string{"already has a recording open", "ao sim flow record stop"} {
		if !strings.Contains(msg, want) {
			t.Fatalf("refusal missing %q:\n%s", want, msg)
		}
	}
...
func TestSimFlowRecordStart_Success(t *testing.T) {
	t.Setenv("AO_SESSION_ID", "mer-9")
	cfg := setConfigEnv(t)
	daemon := newSimDaemon(t, cfg)

	out, errOut, err := executeCLI(t, simLeaseDeps(t, bootedProMaxOnly(t), fakePNG), "sim", "flow", "record", "start", "--name", "sign up")
	if err != nil {
...
func TestSimFlowRecordStatus_NoRecordingSaysSoAndExitsZero(t *testing.T) {
	t.Setenv("AO_SESSION_ID", "mer-9")
	cfg := setConfigEnv(t)
	daemon := newSimDaemon(t, cfg)
	...
	out, errOut, err := executeCLI(t, simLeaseDeps(t, bootedProMaxOnly(t), fakePNG), "sim", "flow", "record", "status")
	if err != nil {
...
func TestSimFlowRecordStatus_OpenRecordingShowsStepCount(t *testing.T) {
	t.Setenv("AO_SESSION_ID", "mer-9")
	cfg := setConfigEnv(t)
	daemon := newSimDaemon(t, cfg)
	...
	out, _, err := executeCLI(t, simLeaseDeps(t, bootedProMaxOnly(t), fakePNG), "sim", "flow", "record", "status")
	if err != nil {
...
func TestSimFlowRecordStatus_JSONCarriesStepCount(t *testing.T) {
	t.Setenv("AO_SESSION_ID", "mer-9")
	cfg := setConfigEnv(t)
	daemon := newSimDaemon(t, cfg)
	...
	out, _, err := executeCLI(t, simLeaseDeps(t, bootedProMaxOnly(t), fakePNG), "sim", "flow", "record", "status", "--json")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	var res simFlowRecordStatusResult
...
func TestSimFlowRecordStop_PrintsThePathTheDaemonWrote(t *testing.T) {
	t.Setenv("AO_SESSION_ID", "mer-9")
	cfg := setConfigEnv(t)
	daemon := newSimDaemon(t, cfg)
	daemon.recordStopBody = stopBody

	out, errOut, err := executeCLI(t, simLeaseDeps(t, bootedProMaxOnly(t), fakePNG), "sim", "flow", "record", "stop")
	if err != nil {
		t.Fatalf("unexpected error: %v\nstderr=%s", err, errOut)
	}
...
// the day it says 3.
func TestSimFlowRecordStop_SaysNothingAboutReviewWhenThereIsNone(t *testing.T) {
	t.Setenv("AO_SESSION_ID", "mer-9")
	cfg := setConfigEnv(t)
	daemon := newSimDaemon(t, cfg)
	daemon.recordStopBody = strings.Replace(stopBody, `"review":2`, `"review":0`, 1)

	out, _, err := executeCLI(t, simLeaseDeps(t, bootedProMaxOnly(t), fakePNG), "sim", "flow", "record", "stop")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestSimFlowRecordStop_JSONCarriesThePathAndBothCounts(t *testing.T) {
	t.Setenv("AO_SESSION_ID", "mer-9")
	cfg := setConfigEnv(t)
	daemon := newSimDaemon(t, cfg)
	daemon.recordStopBody = stopBody

	out, _, err := executeCLI(t, simLeaseDeps(t, bootedProMaxOnly(t), fakePNG), "sim", "flow", "record", "stop", "--json")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	var res simFlowRecordStopResult
	if err := json.Unmarshal([]byte(out), &res); err != nil {
		t.Fatalf("decode: %v\n%s", err, out)
	}
...
// to be running.
func TestSimFlowRecordStop_ForwardsEntryAndAnAbsoluteOut(t *testing.T) {
	t.Setenv("AO_SESSION_ID", "mer-9")
	cfg := setConfigEnv(t)
	daemon := newSimDaemon(t, cfg)
	daemon.recordStopBody = stopBody
	...
	if _, _, err := executeCLI(t, simLeaseDeps(t, bootedProMaxOnly(t), fakePNG),
		"sim", "flow", "record", "stop", "--entry", "../flows/sign-in.yaml", "--out", "custom.yaml"); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
// "nothing happened".
func TestSimFlowRecordStop_NoFlowInTheResponseIsAnError(t *testing.T) {
	t.Setenv("AO_SESSION_ID", "mer-9")
	cfg := setConfigEnv(t)
	daemon := newSimDaemon(t, cfg)
	...
	_, _, err := executeCLI(t, simLeaseDeps(t, bootedProMaxOnly(t), fakePNG), "sim", "flow", "record", "stop")
	if err == nil {
...
func TestSimFlowRecordStop_NoOpenRecordingIsAPlainRefusal(t *testing.T) {
	t.Setenv("AO_SESSION_ID", "mer-9")
	cfg := setConfigEnv(t)
	daemon := newSimDaemon(t, cfg)
	...
	if !strings.Contains(err.Error(), "ao sim flow record start") {
