...
func (c *SessionsController) spawn(w http.ResponseWriter, r *http.Request) {
	...
	sess, err := c.Svc.Spawn(r.Context(), ports.SpawnConfig{ProjectID: in.ProjectID, IssueID: in.IssueID, Kind: in.Kind, Harness: in.Harness, Branch: in.Branch, BaseBranch: in.BaseBranch, Prompt: in.Prompt, DisplayName: displayName})
	if err != nil {
		envelope.WriteError(w, r, err)
		return
	}
