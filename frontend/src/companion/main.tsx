...
function Lab() {
	...
	// Which CHARACTER the whole band is, so the three new ones can be watched walking,
	// talking and being picked up rather than only standing on a contact sheet. Only
	// the lab passes this; a real overlay is every session's own hash and nothing else,
	// which is what keeps the new bodies out of the live cast until they are registered.
	const castFor = useCallback(
		(sessionId: string) => {
			const base = castForSession(sessionId);
			if (species === "proc") return base;
			const chosen = species === "mixed" ? dealSpecies(sessionId) : species;
			return withSpecies(base, chosen);
		},
		[species],
	);
...
/** Deal the four characters round a mixed lab band. Stable per ref, like the real thing. */
