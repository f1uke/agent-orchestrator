...
describe("FileCandidatePicker", () => {
	it("lists every candidate path", () => {
		render(
			<FileCandidatePicker open candidates={["a/x.go", "b/x.go"]} onPick={vi.fn()} onOpenChange={vi.fn()} />,
		);
	...
	it("calls onPick with the chosen path", async () => {
		const onPick = vi.fn();
		render(
			<FileCandidatePicker open candidates={["a/x.go", "b/x.go"]} onPick={onPick} onOpenChange={vi.fn()} />,
		);
