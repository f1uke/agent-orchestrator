...
export function KillSessionButton({ session }: { session: WorkspaceSession }) {
	...
	// which ends the same session by another route and must land the same way.
	const goHome = () => {
	...
	const kill = useMutation({
		...
		onError: (error) => {
			...
			// A refused kill is not a red line under a dialog: it is a different
			// question, with its own answer and its own destructive option. Hand it
			...
			<Dialog.Root open={confirmOpen} onOpenChange={openConfirm}>
				...
				<Dialog.Portal>
					...
					>
						...
						{kill.isError && !(kill.error instanceof UndeliveredWorkError) && (
