 ...
 * marked with a "Last used" chip so a return trip lands where you left off. A fetch
 * failure (e.g. no JIRA_API_TOKEN) surfaces inline rather than an empty list.
 */
			...
			{open ? (
				<div className="jira-proj-picker__drop">
							...
							projects.map((project) => {
								...
								return (
									<button
										...
										className={cn("jira-proj-picker__opt", selected && "is-selected")}
					...
					<div className="jira-proj-picker__foot">
						{projects.length} {projects.length === 1 ? "project" : "projects"} · from jira project list · remembers your
						last pick
					</div>
				</div>
			) : null}
