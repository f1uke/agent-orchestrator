...
describe("stripLeadingJiraKeyPrefix", () => {
	...
	it("strips a leading key + separator, leaving the summary", () => {
		expect(stripLeadingJiraKeyPrefix("STAR-2272 — App eligibility")).toBe("App eligibility");
		expect(stripLeadingJiraKeyPrefix("STAR-2272 - App eligibility")).toBe("App eligibility");
		expect(stripLeadingJiraKeyPrefix("STAR-2272: App eligibility")).toBe("App eligibility");
		expect(stripLeadingJiraKeyPrefix("STAR-2272 App eligibility")).toBe("App eligibility");
	...
	it("strips bracketed / hashed / jira-bound key prefixes", () => {
		expect(stripLeadingJiraKeyPrefix("[STAR-2272] App eligibility")).toBe("App eligibility");
		expect(stripLeadingJiraKeyPrefix("#STAR-2272 App eligibility")).toBe("App eligibility");
		expect(stripLeadingJiraKeyPrefix("jira:STAR-2272 App eligibility")).toBe("App eligibility");
	});

	it("leaves a bare key (no trailing summary) intact", () => {
		expect(stripLeadingJiraKeyPrefix("STAR-2272")).toBe("STAR-2272");
	});
	...
	it("leaves a normal title untouched, including a key that appears mid-string", () => {
		...
		expect(stripLeadingJiraKeyPrefix("Fix flaky test in STAR-2272 flow")).toBe("Fix flaky test in STAR-2272 flow");
	});

	it("does not treat a lowercase or malformed token as a key", () => {
		expect(stripLeadingJiraKeyPrefix("star-2272 nope")).toBe("star-2272 nope");
...
describe("displaySessionName", () => {
	it("uses the display name when it is a clean summary", () => {
		expect(displaySessionName({ displayName: "App eligibility", issueId: "jira:STAR-2272", id: "s1" })).toBe(
			"App eligibility",
		);
	});

	it("strips an accidental key prefix from the display name", () => {
		expect(displaySessionName({ displayName: "STAR-2272 App eligibility", issueId: "jira:STAR-2272", id: "s1" })).toBe(
			"App eligibility",
		);
	});

	it("shows the bare key (not the raw binding) when there is no display name", () => {
		expect(displaySessionName({ displayName: null, issueId: "jira:STAR-2272", id: "s1" })).toBe("STAR-2272");
	});
